...
class TestRequests(unittest.TestCase):
    def setUp(self):
        self.patcher = patch('wrapper.api.request')
    ...
    def test_list_notebooks(self):
        ...
        self.get_asserts(self.api.list_notebooks(),
            'notebooks')
    ...
    def test_create_notebook(self):
        ...
        self.post_asserts(self.api.create_notebook(self.notebook_title),
            data, 'notebooks')

    def test_get_notebook(self):
        self.get_asserts(self.api.get_notebook(self.notebook_id),
            'notebook', self.notebook_id)

    def test_update_notebook(self):
        response = self.api.update_notebook(self.notebook)
        self.put_asserts(response, self.notebook, 'notebook', self.notebook_id)

    def test_remove_notebook(self):
        self.delete_asserts(self.api.remove_notebook(self.notebook_id),
            'notebook', self.notebook_id)

    def test_list_notebook_notes(self):
        self.get_asserts(self.api.list_notebook_notes(self.notebook_id),
            'notes', self.notebook_id)

    def test_create_note(self):
        data = {'title': self.note_title, 'content': self.content}
        self.post_asserts(self.api.create_note(self.notebook_id, self.note_title, self.content),
            data, 'notes', self.notebook_id)

    def test_get_note(self):
        self.get_asserts(self.api.get_note(self.notebook_id, self.note_id),
            'note', self.notebook_id, self.note_id)

    def test_get_notes(self):
        self.get_asserts(self.api.get_notes(self.notebook_id, [self.note_id]),
            'note', self.notebook_id, self.note_id)

    def test_update_note(self):
        self.put_asserts(self.api.update_note(self.note),
            self.note, 'note', self.notebook_id, self.note_id)

    def test_remove_note(self):
        self.delete_asserts(self.api.remove_note(self.note),
            'notes', self.notebook_id, self.note_id)

    def test_remove_notes(self):
        self.delete_asserts(self.api.remove_notes([self.note]),
            'notes', self.notebook_id, self.note_id)

    def test_move_note(self):
        self.get_asserts(self.api.move_note(self.note, self.new_notebook_id),
            'move', self.notebook_id, self.note_id, self.new_notebook_id)

    def test_move_notes(self):
        self.get_asserts(self.api.move_notes([self.note], self.new_notebook_id),
            'move', self.notebook_id, self.note_id, self.new_notebook_id)

    def test_list_note_versions(self):
        self.get_asserts(self.api.list_note_versions(self.note),
            'versions', self.notebook_id, self.note_id)

    def test_list_notes_versions(self):
        self.get_asserts(self.api.list_notes_versions([self.note]),
            'versions', self.notebook_id, self.note_id)

    def test_get_note_version(self):
        self.get_asserts(self.api.get_note_version(self.note, self.version_id),
            'version', self.notebook_id, self.note_id, self.version_id)

    def test_list_note_attachments(self):
        self.get_asserts(self.api.list_note_attachments(self.note),
            'attachments', self.notebook_id, self.note_id, self.version_id)

    def test_get_note_attachment(self):
        self.get_asserts(self.api.get_note_attachment(self.note, self.attachment_id),
            'attachment', self.notebook_id, self.note_id, self.version_id, self.attachment_id)

    def test_remove_note_attachment(self):
        self.delete_asserts(self.api.remove_note_attachment(self.note, self.attachment_id),
            'attachment', self.notebook_id, self.note_id, self.version_id, self.attachment_id)

    # TODO (Nelo Wallus): Fix actual method
    @unittest.expectedFailure
    def test_upload_attachment(self):
        self.post_asserts(self.api.upload_attachment(self.note, self.attachment),
            self.attachment, 'attachments', self.notebook_id, self.note_id, self.version_id)

    def test_list_tags(self):
        self.get_asserts(self.api.list_tags(),
            'tags')

    def test_get_tag(self):
        self.get_asserts(self.api.get_tag(self.tag_id),
            'tag', self.tag_id)

    def test_list_tagged(self):
        self.get_asserts(self.api.list_tagged(self.tag_id),
            'tagged', self.tag_id)

    def test_search(self):
        self.get_asserts(self.api.search(self.keyword),
            'search', wrapper.b64(self.keyword))

    def test_i18n(self):
        self.get_asserts(self.api.i18n(),
            'i18n')

    def test_i18n_param(self):
        self.get_asserts(self.api.i18n(self.keyword),
